...
class EdgeDetector:
    def __init__(self, ImagePath: str):
        ...
        if mode == "Simple":
            self.direction = input(
                "Choose direction: 'H' for horizontal, 'V' for Vertical'.\n"
            )
    ...
    def ConvDetection(self):
        edge = np.zeros(
            (
                self.image.shape[0] - self.operator.shape[0] + 1,
                self.image.shape[1] - self.operator.shape[1] + 1,
                self.image.shape[2],
            )
        )
        for k in range(edge.shape[2]):
            for j in range(edge.shape[1]):
                for i in range(edge.shape[0]):
                    edge[i, j, k] = np.sum(
                        self.image[
                            i : i + self.operator.shape[0],
                            j : j + self.operator.shape[1],
                            k,
                        ]
                        * self.operator
                    )
